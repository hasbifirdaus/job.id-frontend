...
const ApplicantListTable: React.FC<{ jobId: string }> = ({ jobId }) => {
  // Dalam proyek nyata, Anda akan menggunakan state untuk filtering data di sini
  // const [filters, setFilters] = useState({});

  return (
    <div className="space-y-4">
      {/* Filter Pelamar (Filtering Data) */}
      <div className="flex flex-wrap items-center space-x-4">
        ...
        {/* Dropdowns untuk filtering: Usia, Ekspektasi Gaji, Pendidikan */}
        <input
          type="text"
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {DUMMY_APPLICANTS.map((applicant) => (
              <tr key={applicant.id} className="hover:bg-gray-50">
                {/* Kolom Pelamar (Nama & Foto Thumbnail) */}
                ...
                {/* Kolom Aksi (Acceptance Applicant) */}
